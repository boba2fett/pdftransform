...
use crate::{persistence::{files::{FileStorage, GridFSFileStorage}, JobsBasePersistence, MongoPersistenceBase, MongoPreviewPersistence, MongoTransformPersistence}, persistence::{PreviewPersistence, TransformPersistence}, convert::{ConvertService, preview::pdfium::{PreviewService, PdfiumPreviewService}, transform::{PdfiumLibreTransformService}, ConvertServiceImpl}, download::{DownloadServiceImpl}};
...
impl ServiceCollection {
    pub async fn build(mongo_uri: &str, expire_seconds: u64, parallelism: usize, pdfium: Pdfium) -> Result<Self, &'static str> {
        ...
        let preview_persistence = Arc::new(MongoPreviewPersistence {
            base: jobs_base_peristence.clone(),
        });
        let transform_persistence = Arc::new(MongoTransformPersistence {
            base: jobs_base_peristence.clone(),
        });
        let transform_persistence = Arc::new(MongoTransformPersistence {
            base: jobs_base_peristence.clone(),
        ...
        let download_service = Arc::new(DownloadServiceImpl {
            parallelism,
        });
        let convert_service = Arc::new(ConvertServiceImpl {
            base_presistence: jobs_base_peristence.clone(),
