...
use crate::{models::{JobStatus, TransformJobModel, JobLinks, TransformJobDto, CreateTransformJobDto}, routes::transform_job_route};
...
pub async fn get_transform_job_dto(client: &mongodb::Client, job_id: &String, token: String) -> Result<TransformJobDto, &'static str> {
    ...
    Ok(TransformJobDto {
        ...
        _links: JobLinks { _self: transform_job_route(&job_id, &job_model.token) },
        id: job_id,
    })
}
...
pub async fn save_new_transform_job(client: &mongodb::Client, job: TransformJobModel) -> Result<(TransformJobDto, TransformJobModel), &'static str> {
    ...
    if let Ok(insert_result) = jobs.insert_one(job, None).await {
        ...
        return Ok((TransformJobDto {
            ...
            status: JobStatus::InProgress,
            result: vec![],
            _links: JobLinks { _self: transform_job_route(&job_id, &job_clone.token) },
            id: job_id,
        }, TransformJobModel {
            id: Some(id),
            ..job_clone
