...
pub async fn get_result_file(
    ...
    token: &str,
    file_id: &str,
) -> Result<(ContentType, impl Stream<Item = Vec<u8>>), &'static str> {
    let file_model = validate(client, token, file_id).await?;
    let mime_type = file_model.get_content_type();
    let bucket = get_bucket(client);
    let cursor = bucket
        .open_download_stream(file_model.id.unwrap())
        .await
        .map_err(|_| "Could not find file.")?;
    Ok((mime_type, cursor))
}
...
async fn validate(
    ...
    token: &str,
    file_id: &str,
) -> Result<FileModel, &'static str> {
    if let Ok(id) = ObjectId::from_str(file_id) {
        let files = client
            .database(consts::NAME)
            .collection::<FileModel>(FILES_COLLECTION);
        let file = files
...
pub async fn store_result_file(
    ...
    let file_id = bucket
        .upload_from_stream(file_name, source, None)
        .await
        .map_err(|_| "Could not store result.")
        .map(|id| id.to_string())?;
    let chunks = client
        .database(consts::NAME)
        .collection::<DummyModel>(CHUNKS_COLLECTION);
    let chunks_result = chunks.update_many(
        doc! { "files_id": ObjectId::from_str(&file_id).unwrap() },
        doc! {"$set": {"uploadDate": DateTime::now()}},
        None,
    );
    let files = client
        .database(consts::NAME)
        .collection::<DummyModel>(FILES_COLLECTION);
    let file_result = files.update_one(
        doc! { "_id": ObjectId::from_str(&file_id).unwrap() },
        doc! {"$set": {"token": token, "mimeType": mime_type}},
        None,
    );
...
impl TempJobFileProvider {
    ...
    pub async fn clean_up(&self) {
        if let Err(err) = fs::remove_dir_all(&self.job_directory).await {
            warn!(
                "Error occured, while deleting temp job files for {}: {}",
                &self.job_directory.to_str().unwrap_or("<none>"),
                &err
            )
        }
    }
